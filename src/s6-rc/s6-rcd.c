...
#define USAGE "s6-rcd [ -v verbosity ] [ -1 ] [ -C maxconn ] [ -T lameducktimeout ] [ -i rulesdir | -x rulesfile ] [ -c compiled ] [ -l livedir ]"
#define dieusage() strerr_dieusage(100, USAGE) ;

#define S6RC_MAXCONN 64
...
static char const *scandir = S6RC_SCANDIR_DEFAULT ;
static char const *livedir = S6RC_LIVEDIR_DEFAULT ;
...
int main (int argc, char const *const *argv, char const *const *envp)
{
  ...
  int flag1 = 0 ;
  uint32_t maxconn = 4 ;
  char const *compiled = S6RC_COMPILED_DEFAULT ;
  char const *prefix = "" ;
  PROG = "s6-rcd" ;

  {
    ...
    for (;;)
    {
      int opt = subgetopt_r(argc, argv, "v:1C:i:x:t:T:c:l:s:p:", &l) ;
      if (opt == -1) break ;
      switch (opt)
      {
        ...
        case 'l' : livedir = l.arg ; break ;
        case 's' : scandir = l.arg ; break ;
  ...
  if (maxconn > S6RC_MAXCONN) maxconn = S6RC_MAXCONN ;
  if (!maxconn) maxconn = 1 ;
  if (livedir[0] != '/')
    strerr_dief2x(100, livedir, " is not an absolute path") ;
  if (scandir[0] != '/')
    strerr_dief2x(100, scandir, " is not an absolute path") ;
